...
void DStarPlannerROS::initialize(const std::string& name, costmap_2d::Costmap2D* costmap, std::string frame_id){
    ...
    if(!initialized_){
        ...
        tf2_prefix_ = getPrefixParam(prefix_nh);
...
bool DStarPlannerROS::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal, std::vector<geometry_msgs::PoseStamped>& plan){
    ...
    //until tf2 can handle transforming things that are way in the past... we'll require the goal to be in our global frame

    if (ros::names::resolve(tf2_prefix_, goal.header.frame_id) != ros::names::resolve(tf2_prefix_, global_frame)) {

        ROS_ERROR("The goal pose passed to this planner must be in the %s frame.  It is instead in the %s frame.", ros::names::resolve(tf2_prefix_, global_frame).c_str(), ros::names::resolve(tf2_prefix_, goal.header.frame_id).c_str());
        return false;

    }


    if (ros::names::resolve(tf2_prefix_, start.header.frame_id) != ros::names::resolve(tf2_prefix_, global_frame)) {

        ROS_ERROR("The start pose passed to this planner must be in the %s frame.  It is instead in the %s frame.", ros::names::resolve(tf2_prefix_, global_frame).c_str(), ros::names::resolve(tf2_prefix_, start.header.frame_id).c_str());
        return false;
    }
...
void DStarPlannerROS::clearRobotCell(const tf2::Stamped<geometry_msgs::Pose>& global_pose, unsigned int mx, unsigned int my) {
    if (!initialized_) {
        ROS_ERROR("This planner has not been initialized yet, but it is being used, please call initialize() before use");
        return;
    }
...
bool DStarPlannerROS::getPlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal, std::vector<geometry_msgs::PoseStamped>& plan) {
    ...
    tf2::Stamped<geometry_msgs::Pose> start_pose;
